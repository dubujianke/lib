...
//(base) PS D:\study\asm\typescript_chrom_demo> http-server
//(base) PS D:\study\asm\typescript_chrom_demo> http-server OK
//http://127.0.0.1:8080/index.html  OK OK
...
function logMat(mat) {
  ...
  let str = "\n";
  str = str + `${elements[0].toFixed(FIXN)} ${elements[1].toFixed(FIXN)} ${elements[2].toFixed(FIXN)} ${elements[3].toFixed(FIXN)}\n`;
  str = str + `${elements[4].toFixed(FIXN)} ${elements[5].toFixed(FIXN)} ${elements[6].toFixed(FIXN)} ${elements[7].toFixed(FIXN)}\n`;
  str = str + `${elements[8].toFixed(FIXN)} ${elements[9].toFixed(FIXN)} ${elements[10].toFixed(FIXN)} ${elements[11].toFixed(FIXN)}\n`;
  str = str + `${elements[12].toFixed(FIXN)} ${elements[13].toFixed(FIXN)} ${elements[14].toFixed(FIXN)} ${elements[15].toFixed(FIXN)}\n`;
  //console.log(str);
  return str;
}
...
function getMatrixBak(bone) {
  let mat = new THREE.Matrix4();
  mat = mat.premultiply(getLocalMatrixTmp(bone));
  let parent = bone.parent;
  while(parent!= null) {
    let matParent = getLocalMatrixTmp(parent);
    //console.log(parent.name, matParent);
    //console.log(bone.name, mat);
...
function getGlobalMatrix(bone) {
  let mat = matMap[bone.name];
  if(!mat) {
    return bone.matrix;
  }
  return mat;
...
function getMatrix(bone) {
  ...
  let parent = bone.parent;
  if(parent!= null) {
    let matParent = getGlobalMatrix(parent);
    if(!matParent) {
      matMap[bone.name] =  mat;
      return mat;
    }
    //console.log(parent.name, matParent);
    //console.log(bone.name, mat);
...
function getMatrix2(bone) {
  let mat = new THREE.Matrix4();
  //getLocalMatrixTmp
  mat = mat.premultiply(getLocalMatrix(bone));
  let parent = bone.parent;
  while(parent!= null) {
    let matParent = getLocalMatrix(parent);
    //console.log(parent.name, matParent);
    //console.log(bone.name, mat);
    mat = mat.premultiply(matParent);
    //console.log("ret", mat);
...
//real draw
function drawSkeleton_(bone) {
  let mat = getMatrix(bone);
  let bonePoint = new THREE.Vector3();
  bonePoint.setFromMatrixPosition(mat);
  
  let aLocal = getLocalMatrixTmp(bone);
  totalStr = totalStr + "" + bone.name + "\n";
  totalStr = totalStr + logMat(aLocal);
     ...
     loader.parse(buffer.buffer, "", function ( gltf  ) {
      ...
      gltf.scene.traverse(function(obj) {
        ...
        if (obj.isMesh) {
            ...
            let str = ""
            for(let k=0; k<9529; k++) {
              str = str + geometry.attributes.position.getX(k) +","  + geometry.attributes.position.getY(k) +"," + geometry.attributes.position.getZ(k)
              str = str + "\n"
            }
